    ...
    def train(self):
        ...
        for step in range(max_local_epochs):  # FedPRF中限定epochs=1
            for i, (x, y) in enumerate(trainloader):  # load_train_data_minibatch，只有一个batch，循环只有一次
                print(f"Clint {self.id} 的第 {i + 1} 次 iteration, 本次采样个数len(y): {len(y)}")
                ...
                if self.need_adaptive_tau:
                    ...
                    loss_of_global_model = self.loss(output_of_global_model, y)
                    self.loss_global_model = loss_of_global_model.item()
                    loss_of_global_model.backward()
                ...
                if self.need_adaptive_tau:
                    ...
                    loss_of_client_model_batch = self.loss(output_of_client_model_batch, y)
