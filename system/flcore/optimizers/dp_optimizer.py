...
from .utils.dp_utils import cartesian_to_polar, polar_to_cartesian, \
    vector_to_matrix, cartesian_add_noise, devide_epslion, apply_dp_sgd_analysis
...
def make_optimizer_class(cls):
    class DPOptimizerClass(cls):
        ...
        def microbatch_step(self, is_privacy=True, is_auto_s=False, is_fairness=False, fair_coef=1.0):  # 一个样本
            """
            is_privacy：这个函数是可以选False的，但是由于我Optimizer还没改，所以在main里这个参数只能True
            ...
            if is_privacy:  # 如果要进行DP
                if is_auto_s:  # 如果使用AUTO-S正则化裁剪
                    clip_coef = (1.0 / (total_norm + 1e-2))  # 正则化裁剪的裁剪值
                else:  # 用正常的Abadi裁剪
                    clip_coef = min(self.l2_norm_clip / (total_norm + 1e-6), 1.)  # 范数比较，得到等下要裁剪的部分
            if is_fairness:
                clip_coef = clip_coef * fair_coef  # 如果需要公平性的话，就连公平性的coef一起乘掉
            # 梯度的裁剪
            if is_privacy or is_fairness:
